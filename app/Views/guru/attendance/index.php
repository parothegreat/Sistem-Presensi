...
<div class="max-w-7xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-slate-600 mt-1">Kelas: <strong><?= esc($waliKelas['class_name']) ?></strong> | Tanggal: <strong><?= date('d M Y', strtotime($today)) ?></strong></p>
        </div>
        <a href="/guru/dashboard" class="bg-slate-600 hover:bg-slate-700 text-white px-4 py-2 rounded-lg">
            Kembali
        </a>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-4 mb-6">
        ...
        <div class="flex gap-2 items-center">
            <input type="date" id="datePicker" class="px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" value="<?= date('Y-m-d', strtotime($today)) ?>">
            <button onclick="changeDate()" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium">Tampilkan</button>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="p-6 border-b">
            <h2 class="text-lg font-bold text-slate-800">Daftar Absensi Siswa</h2>
        </div>
                            ...
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-6 py-4 text-sm">
                                    <div class="flex gap-2">
                                        <button onclick="markAttendance(<?= $student['id'] ?>, 'on_time')" title="Tepat Waktu" class="text-green-600 hover:text-green-900 font-medium text-xs">
                                            ✓ Tepat
                                        </button>
                                        <button onclick="markAttendance(<?= $student['id'] ?>, 'late')" title="Terlambat" class="text-yellow-600 hover:text-yellow-900 font-medium text-xs">
                                            ⚠ Terlambat
                                        </button>
                                        <button onclick="markAttendance(<?= $student['id'] ?>, 'izin')" title="Izin" class="text-blue-600 hover:text-blue-900 font-medium text-xs">
                                            ⊘ Izin
                                        </button>
                                        <button onclick="markAttendance(<?= $student['id'] ?>, 'sakit')" title="Sakit" class="text-orange-600 hover:text-orange-900 font-medium text-xs">
                                            ⊕ Sakit
                                        </button>
                                        <button onclick="markAttendance(<?= $student['id'] ?>, 'alpha')" title="Alpha" class="text-red-600 hover:text-red-900 font-medium text-xs">
                                            ✗ Alpha
                                        </button>
                                        <a href="/guru/attendance/student/<?= $student['id'] ?>/history" class="text-indigo-600 hover:text-indigo-900 font-medium text-xs">
                                            📋 Riwayat
                                        </a>
                                    </div>
                                </td>
...
<script>
    ...
    function markAttendance(studentId, status) {
        ...
        fetch('/guru/attendance/mark-student', {
                method: 'POST',
                body: formData,
            })
    ...
    function updateStats() {
        fetch('/guru/attendance/get-stats?date=<?= $today ?>')
            .then(response => response.json())
            .then(data => {
                if (data.ok) {
                    // Update stat cards
                    ...
                    document.querySelectorAll('.stat-card').forEach((card, index) => {
                        const values = [stats.total, stats.on_time, stats.late, stats.izin, stats.sakit, stats.alpha];
                        card.querySelector('h3').textContent = values[index];
                    });
                }
            })
    ...
    function changeDate() {
        ...
        if (selectedDate) {
            window.location.href = '/guru/attendance?date=' + selectedDate;
        }
    }
</script>
<?= $this->endSection() ?>