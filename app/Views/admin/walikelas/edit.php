...
<div class="max-w-3xl mx-auto">
    <h1 class="text-2xl font-bold mb-4">Edit Wali Kelas</h1>
    ...
    <form action="<?= base_url('/admin/walikelas/' . $item['id']) ?>" method="post" class="bg-white rounded-lg shadow p-6">
        <?= csrf_field() ?>
        ...
        <div class="mb-4">
            ...
            <select name="teacher_id" class="mt-1 block w-full rounded border px-3 py-2">
                ...
                <?php foreach ($teachers as $t): ?>
                    <option value="<?= $t['id'] ?>" <?= $t['id'] == $item['teacher_id'] ? 'selected' : '' ?>><?= esc($t['full_name']) ?> (<?= esc($t['username'] ?? '') ?>)</option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="class_name" value="<?= esc($item['class_name']) ?>" class="mt-1 block w-full rounded border px-3 py-2" required />
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium text-slate-700">ID Group WhatsApp</label>
            <input type="text" name="wa_group_id" value="<?= esc($item['wa_group_id'] ?? '') ?>" class="mt-1 block w-full rounded border px-3 py-2" placeholder="12345678@g.us" />
