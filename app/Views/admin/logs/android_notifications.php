...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-3xl font-bold text-slate-800"><i class="fab fa-android mr-2 text-green-600"></i>Android Notifications Logs</h1>
            <p class="text-slate-600">Monitor push notifikasi ke aplikasi Android</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <form method="GET" class="grid gap-4 sm:grid-cols-4">
            <div>
                ...
                <input type="date" name="date_from" id="date_from" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-600"
                    value="<?= $filters['date_from'] ?? '' ?>">
            </div>
            <div>
                <label for="date_to" class="block text-sm font-medium text-slate-700 mb-1">Sampai:</label>
                <input type="date" name="date_to" id="date_to" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-600"
                    value="<?= $filters['date_to'] ?? '' ?>">
            </div>
            <div>
                <label for="status" class="block text-sm font-medium text-slate-700 mb-1">Status:</label>
                <select name="status" id="status" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-600">
            ...
            <div class="flex items-end gap-2">
                <button type="submit" class="flex-1 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg transition flex items-center justify-center gap-2">
                    <i class="fas fa-search"></i> Cari
                </button>
                <a href="<?= base_url('/admin/logs/android') ?>" class="bg-slate-300 hover:bg-slate-400 text-slate-800 px-4 py-2 rounded-lg transition">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4">
            <h3 class="text-lg font-semibold text-white"><i class="fas fa-history mr-2"></i>Daftar Android Notifications Logs</h3>
        </div>
        <div class="p-6">
            <?php if (!empty($logs)): ?>
                <div class="overflow-x-auto">
                    <table id="androidTable" class="w-full display">
...
<script>
    $(document).ready(function() {
        var table = $('#androidTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: '<?= base_url('admin/logs/android-json') ?>',
                data: function(d) {
        ...
        $('a[href*="android"]').on('click', function(e) {
            if ($(this).attr('href').endsWith('android')) {
                e.preventDefault();
                $('form')[0].reset();
                table.ajax.reload();
            }
