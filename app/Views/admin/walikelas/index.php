...
<div class="max-w-7xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold">Kelola Wali Kelas</h1>
        <a href="<?= base_url('/admin/walikelas/create') ?>" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded">Buat Wali Kelas</a>
    </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="overflow-x-auto">
            <table id="walikkelasTable" class="w-full display align-middle">
                <thead class="bg-slate-50 border-b">
                    <tr>
                        <th class="px-6 py-4 font-semibold text-slate-700 text-left uppercase text-xs tracking-wider">Kelas</th>
                        <th class="px-6 py-4 font-semibold text-slate-700 text-left uppercase text-xs tracking-wider">Wali Kelas</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php foreach ($walikelas as $w): ?>
                        <tr class="hover:bg-slate-50 transition duration-150">
                            <!-- Kelas Column -->
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-4 py-1.5 rounded-full text-sm font-semibold bg-indigo-100 text-indigo-800 border border-indigo-200 shadow-sm">
                                    <?= esc($w['class_name']) ?>
                                </span>
                            </td>

                            <!-- Wali Kelas Column (Photo + Name) -->
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <?php if (!empty($w['photo'])): ?>
                                            <img class="h-10 w-10 rounded-full object-cover border border-slate-200" src="<?= base_url($w['photo']) ?>" alt="<?= esc($w['teacher_name']) ?>">
                                        <?php else: ?>
                                            <img class="h-10 w-10 rounded-full border border-slate-200" src="https://ui-avatars.com/api/?name=<?= urlencode($w['teacher_name'] ?? 'Wali Kelas') ?>&background=eff6ff&color=2563eb&bold=true" alt="<?= esc($w['teacher_name'] ?? 'WK') ?>">
                                        <?php endif; ?>
                                    </div>
                                    <div class="ml-4">
                                        <div class="text-sm font-semibold text-slate-900"><?= esc($w['teacher_name'] ?? '-') ?></div>
                                        <div class="text-xs text-slate-500 font-mono">NIP: <?= esc($w['nip'] ?? '-') ?></div>
                                    </div>
                                </div>
                            </td>
...
<style>
    ...
    #walikkelasTable thead {
        background-color: #f1f5f9;
    }

    #walikkelasTable tbody tr {
        border-bottom: 1px solid #e2e8f0;
    }

    #walikkelasTable tbody tr:hover {
        background-color: #f8fafc;
    }

    /* Align last column (Aksi) to right */
    #walikkelasTable thead th:last-child {
        text-align: right !important;
        padding-right: 2rem !important;
    }

    #walikkelasTable tbody td:last-child {
        text-align: right !important;
        padding-right: 2rem !important;
    }
