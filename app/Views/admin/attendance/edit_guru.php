...
<div class="max-w-2xl mx-auto">
    <!-- Header -->
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-slate-800">Edit Absensi Guru</h1>
        <p class="text-slate-600">
            <strong><?= esc($attendance['username']) ?></strong> -
    ...
    <!-- Form -->
    <form method="post" action="<?= base_url('/admin/attendance/guru/' . $attendance['id']) ?>" class="bg-white rounded-lg shadow p-6">
        <?= csrf_field() ?>
...
<?= esc($attendance['note'] ?? '') ?></textarea>
        ...
        <div class="flex gap-3">
            ...
            <a href="<?= base_url('/admin/attendance/insert-guru') ?>" class="px-6 py-2 bg-slate-200 text-slate-700 rounded-lg hover:bg-slate-300 font-medium">
                Batal
            </a>
        </div>
    </form>
