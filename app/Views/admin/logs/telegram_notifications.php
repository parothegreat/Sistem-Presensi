...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-3xl font-bold text-slate-800"><i class="fab fa-telegram mr-2 text-blue-600"></i>Telegram Notifications Logs</h1>
            <p class="text-slate-600">Monitor push notifikasi ke Telegram</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <form method="GET" class="grid gap-4 sm:grid-cols-4">
            <div>
                ...
                <input type="date" name="date_from" id="date_from" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600"
                    value="<?= $filters['date_from'] ?? '' ?>">
            </div>
            <div>
                <label for="date_to" class="block text-sm font-medium text-slate-700 mb-1">Sampai:</label>
                <input type="date" name="date_to" id="date_to" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600"
                    value="<?= $filters['date_to'] ?? '' ?>">
            </div>
            <div>
                <label for="status" class="block text-sm font-medium text-slate-700 mb-1">Status:</label>
                <select name="status" id="status" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">
            ...
            <div class="flex items-end gap-2">
                <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition flex items-center justify-center gap-2">
                    <i class="fas fa-search"></i> Cari
                </button>
                <a href="<?= base_url('/admin/logs/telegram') ?>" class="bg-slate-300 hover:bg-slate-400 text-slate-800 px-4 py-2 rounded-lg transition">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
            <h3 class="text-lg font-semibold text-white"><i class="fas fa-history mr-2"></i>Daftar Telegram Notifications Logs</h3>
        </div>
        <div class="p-6">
            <?php if (!empty($logs)): ?>
                <div class="overflow-x-auto">
                    <table id="telegramTable" class="w-full display">
                        <thead class="bg-slate-100 border-b">
                            <tr>
                                <th class="text-left px-4 py-3 font-semibold text-slate-700">Waktu Buat</th>
                                <th class="text-left px-4 py-3 font-semibold text-slate-700">Chat ID</th>
                                ...
                                <th class="text-left px-4 py-3 font-semibold text-slate-700">Scheduled At</th>
...
<script>
    $(document).ready(function() {
        var table = $('#telegramTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: '<?= base_url('admin/logs/telegram-json') ?>',
                data: function(d) {
        ...
        $('a[href*="telegram"]').on('click', function(e) {
            if ($(this).attr('href').endsWith('telegram')) {
                e.preventDefault();
                $('form')[0].reset();
                table.ajax.reload();
            }
